...
One report: before → after numbers from step 1, then `applied`, `reverted`, `proposed`, `dropped` (with REFUTED/PLAUSIBLE counts), and `bugs noticed` for the user to take to `/octo-review`. A clean target is a valid outcome — never invent findings.

## Angles

Each `###` below is one angle; on a large target each angle is one finder. Add an angle here and it is covered with no edit to the steps.
