...
- `global-codex-wrapper.sh` — installs to `~/.local/bin/codex`; checks for a Codex update on every launch, and keeps the npm package in user-owned `~/.local/share/octo-codex`. It passes no hook-trust bypass — hooks run because `global-codex-config.toml` carries their reviewed hashes.
