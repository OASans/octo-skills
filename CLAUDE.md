...
- `global-codex-config.toml` — managed Codex approval, sandbox, context-window, auto-compact, and `[tui]` status-line defaults. install.sh merges only those keys into `~/.codex/config.toml` (Codex owns that file — theme, trusted projects, hook trust hashes — so it's spliced, never overwritten).
