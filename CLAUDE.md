...
- `global-codex-wrapper.sh` — installs to `~/.local/bin/codex`; checks for a Codex update on every launch, keeps the npm package in user-owned `~/.local/share/octo-codex`, and bypasses hook-trust checks because this machine is fully owned.
...
- `install.sh` — the single install path for both agents: copies `skills/*` to `~/.claude/skills/` and `~/.codex/skills/`, installs the shared prompts and settings (including the complete Codex config), derives Codex hooks from `global-settings.json`, then installs Node.js+npm and Codex under user-owned `~/.local/share/octo-codex`. Idempotent — always safe to run directly. Deploy any edit by running `./install.sh`, not by hand-copying files; its Node / Codex-CLI / Swift-LSP / Playwright steps are guarded no-ops when already present, and a failed dependency install warns rather than aborting.
