...
- `install.sh` — the single install path for both agents: copies `skills/*` to `~/.claude/skills/` and `~/.codex/skills/`, installs the shared prompts and settings, derives Codex's Git Sync hook, and installs OpenAI's standalone Codex package. `~/.local/bin/codex` points directly to the official executable; no wrapper modifies its arguments. Idempotent — always safe to run directly.
...
| `/octo-simplify` | Workflow (manual) | Simplify code for the same behavior: fans out one finder per `###` angle, verifies removal claims, applies safe changes under the build/test gate, proposes risky ones for approval. Slash-only; never auto-runs |
...
- `/octo-simplify` is the **fixing counterpart of `/octo-review`**: both judge against the `octo-coding-guide-*` family, but review reports and simplify edits. It is slash-only (`disable-model-invocation: true`) because it deletes code, and stays out of the `## Workflow`; its output is reviewed by `/octo-review` like any other change. Structure-driven by its own `###` angles — one finder sub-agent each.
