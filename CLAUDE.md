...
- `.claude/skills/` — this project's own skills (the `knowledge-*` topics maintained by the memory skills), and the source of truth for them. `.codex/skills` is a committed symlink to this directory, so both agents load one set. Same rule for any project: keep the skills in `.claude/skills/` and symlink `.codex/skills -> ../.claude/skills`.
