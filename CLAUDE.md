...
- `global-settings.json` — shared Claude Code settings (env vars, permissions, hooks). Installs to `~/.claude/settings.json`. Also the single source for Codex's `~/.codex/hooks.json`: install derives it via `jq` (Codex's own config format is TOML, so the rest does NOT port — only the plain git/shell `SessionStart` hook does; the Claude-only hooks depend on env vars Codex doesn't set).
...
- `install.sh` — the single install path for both agents: copies `skills/*` to `~/.claude/skills/` and `~/.codex/skills/`, installs `global-CLAUDE.md` as Claude `CLAUDE.md` + Codex `AGENTS.md`, `global-settings.json` as Claude `settings.json` and (jq-derived) Codex `hooks.json`, then installs the Codex CLI (`npm i -g @openai/codex`). Idempotent — always safe to run directly. Deploy any edit by running `./install.sh`, not by hand-copying files; its Codex-CLI / Swift-LSP / Node-Playwright steps are guarded no-ops when those are already present.
