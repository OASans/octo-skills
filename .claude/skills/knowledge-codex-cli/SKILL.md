...
- **Linux Remote Control uses one app-server:** systemd runs `octo-codex-remote-control.service` at `$XDG_RUNTIME_DIR/octo-codex/app-server.sock`. Use `codex --remote unix://$XDG_RUNTIME_DIR/octo-codex/app-server.sock` when a TUI should connect to that shared server; a plain `codex` starts a separate local session.
...
- `global-codex-remote-control.service` — the single boot-persistent Linux app-server.
