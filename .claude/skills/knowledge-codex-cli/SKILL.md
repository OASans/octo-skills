...
- **Hook trust bypass is launch-only and always warns:** `--dangerously-bypass-hook-trust` bypasses persisted hook trust for one invocation, and Codex prints a TUI warning every time it is present. Persisted trust does not hide that warning; the only banner-free launch is without the flag, trusting definitions via `/hooks` (stderr filtering does not work — the TUI renders it). `global-codex-wrapper.sh` therefore does **not** pass the flag; hooks run because `global-codex-config.toml` carries their reviewed hashes.
...
- `global-codex-wrapper.sh` — auto-updates the managed package; launches Codex with no hook-trust bypass.
