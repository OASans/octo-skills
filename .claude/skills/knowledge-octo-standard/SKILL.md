---
name: knowledge-octo-standard
description: >
  Editing octo-standard: dimension→rule→nested-criteria structure, scope, and
  rule-writing conventions. Load before editing skills/octo-standard/SKILL.md.
user-invocable: false
---

# Editing the octo-standard Skill

## What
`octo-standard` defines a good **AI-agent-native package** and reviews a target package against it, emitting prioritized action items. **Explicitly-invoked only** (`disable-model-invocation: true`) — never auto-runs.

`skills/octo-standard/SKILL.md` has three parts:
...
- **The Standard** — the rules, as `###` **dimensions** (e.g. `### CLAUDE.md`), governed by a `> Structure contract` blockquote. The review walks every rule in every dimension.
...
- **Deploy**: edit the source `skills/octo-standard/SKILL.md`, then `./install.sh`; never hand-edit `~/.claude/skills/`.

## Key Files
- `skills/octo-standard/SKILL.md` — the skill; `### CLAUDE.md` is the worked example to copy.
