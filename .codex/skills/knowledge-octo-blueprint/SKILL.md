...
- **Rule format**: top-level bullet (`**Name** — essence`) with checkable criteria as **nested sub-bullets**. Group rules within a dimension when kinds differ (the AGENTS.md dimension splits *Sections — in order* vs *Whole-file format*).
- **Scope**: governs a package's **project-level `AGENTS.md`**, NOT the global `~/.codex/AGENTS.md` (different logic — exempt).
- **Conventions to reuse** (set in the AGENTS.md dimension): prefer runnable paths over alias layers; hardcode shared **verbatim** boilerplate lines across packages for fixed directives; **verify each claim against the real artifact** (read the script / run `--help`), don't trust the text; name commit by the **action word, never the skill name** (skills may be renamed).
